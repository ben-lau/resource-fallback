...
每次重试/回退都创建全新的 `<script>` 元素，绕过浏览器缓存。

#### data-webpack ownership

重试/回退创建的 `<script>` 元素会设置 `data-webpack` 属性（值为 chunk loading key）。Observer 自动跳过带 `data-webpack` 属性的 `<script>` 标签，避免与 webpack adapter 重复处理同一次失败。
...
`mini-css-extract-plugin` 输出的 CSS chunk 以 `<link>` 标签形式注入，虽然也带 `data-webpack` 属性，但 webpack adapter **不拦截 CSS 加载**。Observer 仍然是 CSS chunk 的唯一安全网，必须继续处理 CSS `<link>` 的失败与替换，避免 `Promise.all` 因 CSS chunk promise 被 reject 而提前短路。
