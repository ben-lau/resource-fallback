...
/* -----------------------------------------------------------------------
 * 方案 A（已弃用）—— 完全接管 instantiate，自建 <script> 元素
 * -----------------------------------------------------------------------
 *
 * 此方案覆写 `System.constructor.prototype.instantiate`，不再委托给原始
 * 实现，而是自行创建 `<script>` 元素，并通过 `data-systemjs` 属性标记，
 * 让 observer 跳过这些脚本（避免双重处理）。
 *
 * 优点：
 *   - 完全控制脚本创建、事件绑定和重试逻辑
 *   - 不存在 observer 与 adapter 的竞争
 *
 * 缺点：
 *   - 需要在 observer 中增加 `isSystemJSScript` 检查
 *   - 自建脚本可能遗漏 SystemJS 内部附加的属性（crossOrigin、fetchPriority 等）
 *   - 如果 SystemJS 更新了 instantiate 的内部逻辑（如 integrity 校验、
 *     import map 支持），自建脚本不会自动获得这些改进
 *
 * 实现代码（hookInstantiate 部分）：
 *
 *   function hookInstantiate(proto, deps) {
 *     const origInstantiate = proto.instantiate;
 ...
 *       }
 *       const sys = this;
 *       let currentUrl = url, attempt = 1, isFallback = false;
 *
 ...
 *           script.async = true;
 *           script.setAttribute('data-systemjs', '1');
 ...
 *             const result = deps.resolver.resolve(currentUrl, attempt, isFallback);
 *             if (result.kind === 'giveup') {
 *               reject(new Error('Failed to load SystemJS module: ' + url));
 *               return;
 *             }
 *             if (result.kind === 'retry') {
 *               attempt = result.attempt + 1;
 ...
 *             }
 *             currentUrl = result.url;
 *             const doRetry = () => loadWithFallback().then(resolve, reject);
 *             if (result.delay > 0) setTimeout(doRetry, result.delay);
 *             else doRetry();
 *           };
 *
 ...
 *   }
 *
 * 需要配合 observer.ts 中的检查：
 *
 *   function isSystemJSScript(el) {
 *     return el.tagName === 'SCRIPT' && el.hasAttribute('data-systemjs');
 *   }
 *   // 在 onError 中: if (isSystemJSScript(el)) return;
 *
 * ----------------------------------------------------------------------- */
