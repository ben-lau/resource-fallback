...
describe('resolver', () => {
  ...
  it('throws when base is missing or not a string', () => {
    expect(() =>
      createResolver({
        rules: [{ base: '' as string, urls: [cdn1] }],
      }),
    ).toThrow(/base must be a non-empty string/);

    expect(() =>
      createResolver({
        // @ts-expect-error intentional invalid config
        rules: [{ urls: [cdn1] }],
      }),
    ).toThrow(/base must be a non-empty string/);
  });

  it('normalizes missing trailing slash on base and urls', () => {
    const r = createResolver({
      rules: [
        {
          base: 'https://cdn1.example.com',
          urls: ['https://cdn2.example.com', origin.replace(/\/$/, '')],
          retry: { max: 0, baseDelay: 0, maxDelay: 0, jitter: false },
        },
      ],
    });
    const result = r.resolve('https://cdn1.example.com/x.js', 1);
    expect(result.kind).toBe('fallback');
    if (result.kind === 'fallback') {
      expect(result.url).toBe('https://cdn2.example.com/x.js');
    }
  });

  it('resolveBuiltUrl prefers the base URL over urls', () => {
    const r = build();
    expect(r.resolveBuiltUrl('foo.js')).toBe(cdn1 + 'foo.js');
  });

  it('resolveBuiltUrl inserts slash between base without trailing slash and nested filename', () => {
    const r = createResolver({
      rules: [
        {
          base: 'https://qn.cache.wpscdn.cn/fe/edu/edu-study-platform-prod',
  ...
  it('resolveBuiltUrl falls back to filename if no rules', () => {
    const r = createResolver({ rules: [] });
    expect(r.resolveBuiltUrl('foo.js')).toBe('foo.js');
  });

  it('handles base ≠ urls[0] correctly', () => {
    const primary = 'https://cdn.original.com/';
    ...
    const url = primary + 'assets/index.js';
  ...
  it('isFallback=false: only matches via base prefix, not urls prefix', () => {
    const r = build();
    // cdn2 is in the urls list but doesn't match the base (cdn1)
  ...
  it('initial link failure records to breaker but resolve() still matches via base', () => {
    ...
    // But resolve() still finds the rule via base — initial link is always tried.
  ...
  it('findRule returns the rule for matched URLs', () => {
    ...
    expect(r.findRule(cdn1 + 'foo.js')?.base).toBe(cdn1);
  });
  ...
  it('resolveBuiltUrl with duplicate base: returns last rule base', () => {
    const r = createResolver({
      rules: [
        {
          base: cdn1,
  ...
  it('resolve with duplicate base: last rule takes precedence', () => {
    const r = createResolver({
      rules: [
        {
          base: cdn1,
  ...
  it('handles base ≠ urls when URL matches base but not any urls entry', () => {
    const primary = 'https://cdn.original.com/';
    ...
    const url = primary + 'assets/chunk.js';
    const result = r.resolve(url, 1);
    expect(result.kind).toBe('fallback');
    if (result.kind === 'fallback') {
